...
function reservingSubscription() {
  ...
  if (res) {
    showDialog("ReservingSubscription", res);
  }
}
...
function makeStatistic() {
  ...
  if (sheetAbon.getLastColumn() <= 1 && sheetAbonArch.getLastColumn() <= 1) {
    return;
  } // если пустой журнал абонементов - закончили.
  var abonsData = [];
  var abonsArchData = [];
  if (sheetAbon.getLastRow() > 1) {
    abonsData = sheetAbon
      .getRange(2, 1, sheetAbon.getLastRow() - 1, sheetAbon.getLastColumn())
      .getValues();
  }
  Logger.log(abonsData);
  if (sheetAbonArch.getLastRow() > 1) {
    abonsArchData = sheetAbonArch
      .getRange(
        2,
        1,
        sheetAbonArch.getLastRow() - 1,
        sheetAbonArch.getLastColumn()
      )
      .getValues();
  }

  var allAbons = abonsData.concat(abonsArchData);
  if (allAbons.length == 0) {
    return;
  }

  var sheetSpravochnik = ss.getSheetByName("Справочник");
  var nachislenia = sheetSpravochnik
    .getRange(2, 6, sheetSpravochnik.getLastRow() - 1, 3)
    .getValues();
  ...
  if (date instanceof Date) {
      ...
      for (var j = 0; j < q; j++) {
        if (allAbons[i][11 + j * 5] == teacherName) {
          ...
          if (lessonDate instanceof Date) {
            var lessonMonthYear =
              "" + (lessonDate.getMonth() + 1) + "" + lessonDate.getFullYear();
    ...
    sheetVipiska.appendRow([
      ...
      "Сумма",
      "",
      "=count(F4:F" + sheetVipiska.getMaxRows() + ")",
      "",
      "=sum(H4:H" + sheetVipiska.getMaxRows() + ")",
      ""
    ]);
    sheetVipiska
      .getRange(sheetVipiska.getLastRow(), 1, 1, sheetVipiska.getMaxColumns())
      .setBackgrounds([
        [
          "#D9D9D9",
          "#D9D9D9",
